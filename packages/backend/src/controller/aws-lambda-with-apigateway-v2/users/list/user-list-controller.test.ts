...
import { StubUserListService } from '../../../../application/users/list/stub-user-list-service';
...
const userListService = new StubUserListService();
...
describe('ユーザー一覧取得', () => {
  test('ユーザー一覧を取得できる', async () => {
    ...
    jest
      .spyOn(StubUserListService.prototype, 'handle')
    ...
    const response = await userListController.handle({
  ...
  test('limitにnumber型に変換できない値が設定されているとBadRequestを返す', async () => {
    jest.spyOn(StubUserListService.prototype, 'handle');
